 ...
 * @throws TaskError when the instance doesn't have the shortHash in the name
 */
function extractIsolatedMasterShortHash (instanceName) {
  var matches = regexForIsolatedNames.exec(instanceName)
  if (!matches.length) {
    // If this happens, we've hit a bug
    throw new TaskFatalError(
      'extractIsolatedMasterShortHash',
...
function getIsolatedShorthash (instance) {
  if (instance.attrs.isolated && !instance.attrs.isIsolationGroupMaster) {
    return extractIsolatedMasterShortHash(instance.attrs.name)
  }
        ...
        if (dependencies) {
          dependencies.forEach(function (dependency) {
            ...
            if (dependency.isolated && regexForIsolatedNames.test(dependency.name)) {
              newDep.isolatedMastersShortHash = extractIsolatedMasterShortHash(dependency.name)
            }
