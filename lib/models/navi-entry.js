...
NaviEntrySchema.statics.handleInstanceUpdate = function (instance, timestamp) {
  ...
  return NaviEntrySchema.statics._getDirectURlObj(instance, timestamp)
    .then(function (directUrlObj) {
      ...
      var elasticHostname = instance.getElasticHostname()
      var find = {
        elasticUrl: elasticHostname,
        $or: [timestampCheck, existsCheck]
      }
      return Promise.fromCallback(function (callback) {
        if (!keypather.get(instance, 'attrs.ipWhitelist.enabled')) {
          var update = {
            $set: {
              elasticUrl: elasticHostname,
              ownerGithubId: instance.attrs.owner.github,
              'ipWhitelist.enabled': keypather.get(instance, 'attrs.ipWhitelist.enabled') || false
            }
          }
          ...
          NaviEntry.findOneAndRemove(find, callback)
        }
      })
        .then(function (updateResults) {
          if (updateResults) {
            hermesInstance.publishCacheInvalidated(elasticHostname)
          }
