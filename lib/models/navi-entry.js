      ...
      function processDependencies (dependencies) {
        ...
        if (dependencies) {
          dependencies.forEach(function (dependency) {
            ...
            if (keypather.get(dependency, 'instance.attrs.isolated')) {
              newDep.isolatedShorthash = newDep.shortHash
              var index = dependency.hostname.indexOf('--') + 1
              newDep.shortHash = dependency.hostname.slice(0, index)
              newDep.elasticUrl = dependency.hostname.slice(index + 1)
            }
